...
class MissionsController extends Controller
{
    ...
    public function addMission(Request $request){
        ...
        $validator = Validator::make($input, [
            'title' => 'required_if:categoryId,1',
            // 'desc' => 'required',
            'startedAt' => 'required',
            'categoryId' => 'required',
            'peopleId' =>'required_unless:categoryId,1'
        ],[
            'title.required_if' => 'يرجى إدخال العنوان',
            'desc.required' => 'يرجى إدخال الموضوع',
            'startedAt.required' => 'يرجى إدخال تاريخ البدء',
            'peopleId.required_unless' => 'يرجى تحديد الضابط أو الفرد'
        ]);

        is_array(json_decode($request->peopleId)) ? '' : abort(500);
        ...
        if( $request->categoryId == 1 || count(json_decode($request->peopleId)) ){
            $mission->people()->sync(json_decode($request->peopleId));
        }
